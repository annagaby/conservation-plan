...
# Download specific layers to current directory. In this case we are downloading mean sea surface temperature, max sea surface temperature, min sea surface temperature, dissolved oxygen, mean chlorophyl A and sea surface salinity
files <- load_layers(c("BO_sstmean", "BO_sstmax", "BO_sstmin", "BO_dissox", "MS_biogeo08_sss_mean_5m", "BO_chlomean")) 
...
me_masked <- mask(x = cobia_pred, mask = eez)
plot(me_masked)

# Crop sst range to Brazil's EEZ extent
me_cropped <- crop(x = me_masked, y = extent(eez))
plot(me_cropped)
