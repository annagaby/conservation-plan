...
layer_name <- c("Depth", "Min SST", "Max SST", "Distance to Shore", "MPAs", "Artificial Reefs", "Reefs", "Wetlands", "No-take zones")

# Create vector with number of 0 cells in raster
freq_0 <- c(freq(depth_binary, value = 0), freq(sst_binary_min, value = 0), freq(sst_binary_max, value = 0), freq(dist_shore_binary, value = 0), freq(mpas_binary, value = 0), freq(reefs_artificial_binary, value = 0), freq(reefs_binary, value = 0), freq(wetlands_binary, value = 0), freq(no_take_binary, value = 0))
...
area_col <- ggplot(area_df, aes(x = layer_name, y = percent_excluded)) +
  ...
  theme_classic(14) +
  geom_text(
    aes(label = percent_excluded, y = percent_excluded + 10), 
    color = "black", 
    size = 5,
    hjust = 1
  ) 
